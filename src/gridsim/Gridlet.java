 ...
 *
 * Gridlet是一个包，其中包含与该作业及其执行管理详细信息有关的所有信息，
 * 例如以MI（百万指令）表示的作业长度，输入和输出文件的大小以及作业所有者ID。
 * 各个用户通过创建用于在Grid资源上对其进行处理的Gridlet对其应用程序进行建模。
 * 这些基本参数有助于确定执行时间，在用户和远程资源之间传输输入和输出文件所需的时间，
 * 以及将处理后的Gridlets与结果一起返回给发起者。
 *
 * 对于需要许多处理元素（PE）或CPU的Gridlet，为简单起见，仅计算1个PE的Gridlet长度。
 * 例如，此Gridlet的长度为500 MI，需要2个PE。这意味着每个PE将执行此Gridlet的500 MI。
 *
 * 从GridSim 3.1开始，我们还添加了classType属性，该属性可用于提供区分服务以调度资源上的网格小程序。
 * classType越高，意味着在资源上调度此Gridlet的优先级越高。但是，由资源分配策略决定是否根据优先级调度小网格。
 *
 * 默认情况下，此对象记录每个活动的历史记录，例如何时将此Gridlet放入队列中以及何时由资源执行它。
 * 您可以使用getGridletHistory（）查找历史记录。
 * 但是，如果您有许多Gridlet对象，则此方法需要大量内存。
 * 要禁用此功能，请改用Gridlet（int，double，long，long，boolean）构造函数。
 *
