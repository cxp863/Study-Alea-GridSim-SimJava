 ...
 * </ul>
 *
 * 该类主要负责整体模拟的初始化，运行和停止。
 * 在用户级别创建任何其他GridSim实体之前，必须初始化GridSim以设置仿真环境。
 * 通过创建三个GridSim内部实体-GridInformationService，GridSimShutdown，GridStatistics，此方法还为仿真准备了系统。
 * 调用startGridSimulation()方法将启动Grid仿真。在调用以上两种方法之间必须实例化所有资源和用户实体。
 *
 * 从GridSim 3.0版开始，所有I/O方法都已移入GridSimCore。
 * 因此，此类仅专注于记录统计信息和管理Gridlet。此外，还有三种不同的初始化GridSim模拟的方法。这些方法是：
 *
 * 通过使用init（int，Calendar，boolean，String []，String []，String）方法。
 * 此方法将创建GridStatistics，GridSimRandom，GridSimShutdown和GridInformationService实体。
 * 通过使用init（int，Calendar，boolean）方法。
 * 此方法将创建GridSimRandom，GridSimShutdown和GridInformationService实体。
 * 通过使用init（int，Calendar，boolean，boolean）方法。
 * 此方法将创建GridSimRandom和GridSimShutdown。
 * 在运行模拟之前，需要使用setGIS（GridInformationService）方法输入不同类型的GridInformationService实体。
 *
