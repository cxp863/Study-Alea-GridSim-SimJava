 ...
 * </ul>
 *
 * 数据网格资源使用户能够运行其作业以及访问可用数据集。数据网格资源具有以下组件：
 * 存储：资源可以具有一个或多个不同的存储元素，例如硬盘或磁带驱动器。单个存储负责存储，检索和删除文件。
 *
 * 副本管理器：此组件负责处理和管理有关资源中一个或多个存储元素的数据集的传入请求。
 * 它还将存储在资源中的文件注册到指定的RC。
 *
 * 本地副本目录（RC）：此组件是资源的可选部分。本地RC仅负责索引资源上的可用文件。
 * 它还处理用户的查询。但是，本地RC不能用作其他资源的目录服务器。这应该由分支机构或区域RC完成。
 *
 * 分配策略：此组件负责对资源中一个或多个可用节点执行用户作业。
 *
