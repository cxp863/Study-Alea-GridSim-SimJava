 ...
 *
 * 此类处理所有高级保留（AR）功能，例如创建，修改，取消和查询。保留有两种类型：
 * 高级预订：要求在将来的时间进行预订。
 * 立即预订：立即请求预订，即使用当前时间作为开始时间，并指定是否指定持续时间或结束时间。
 * 在此类中，可以通过以下方式之一立即预订：
 * 开始时间= 0（以长整数表示）或null（在Calendar对象中）且持续时间> 0。
 * 如果成功，则资源设置的到期时间将是：到期时间=当前时间+持续时间。
 *
 * 开始时间= 0（以长整数表示）或null（在Calendar对象中）且持续时间= 0。
 * 这意味着只要有可用的PE，就在运行保留。
 * 注意：使用这种方法，当新的AR请求到来时，预留有被资源调度程序抢占或终止的风险。另外，由于复杂性，资源的调度程序可能不支持此方法。最后，到期时间= 0，因为资源的调度程序无法确定它。
 *
 * 除了使用这种方法，您还可以使用GridSim.gridletSubmit（Gridlet，int）方法直接提交作业或Gridlet对象。
 * 此类中的AR方法自动处理与目的地网格资源实体之间的通信。要使用AR功能，您需要创建此对象的子类，即创建一个继承该对象的新类。这种方法是可取的，因为子类负责从资源实体收集回Gridlet。
 *
 * 关于此实体的重要属性是时间和时区。此实体中的时间，GridResource实体和模拟时钟都与给定的初始时间有关。换句话说，将来的时间应该大于在GridSim.init（int，Calendar，boolean）方法期间给定的初始化时间。
 * 想象以下情况：
 *
 * 模拟初始化时间：Calendar cal = Calendar.getInstance（）;
 * 假设时间和日期为2004年5月2日16:00:00。
 * 模拟时钟：双时钟= GridSim.clock（）;
 * 假设模拟已经运行了45秒（根据模拟或SimJava时钟而不是GridSim的实际运行）。因此，当前的模拟时间和日期是2004年5月2日16:00:45。
 * 一个用户实体在2004年5月2日15:00:00保留一个时段。
 * 结果：由于时间已过，因此立即被拒绝。
 * 用户实体在2004年5月2日16:10:00保留时段。
 * 结果：根据资源可用性接受或拒绝。如果接受，则假定保留的有效期是从当前时间开始5分钟，那么用户必须在2004年5月2日16:05:45的时间和日期之前提交。
 * 时间可以表示为Calendar对象或以毫秒为单位的时间。
 *
 * 用户和GridResource实体可能具有不同的时区。因此，当这两个实体之间进行通信时，到达时，时间将由单个实体转换为其本地时间。
 *
