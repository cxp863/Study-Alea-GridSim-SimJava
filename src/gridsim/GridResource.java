 ...
 *
 * GridResource扩展了GridSimCore类以获得通信和并发实体功能。
 * 该类的实例模拟具有在ResourceCharacteristics类的对象中定义的属性的资源。
 * 创建网格资源的过程如下：
 *
 * 创建具有合适的MIPS（每秒百万条指令）或SPEC（标准性能评估公司）等级的PE（处理元素）对象；
 * 将它们组装在一起以创建机器；
 * 将机器的一个或多个对象分组以形成Grid资源
 * 使用循环调度算法，将具有一台机器并具有一个或多个PE（处理元素）的资源作为分时系统进行管理。
 * 具有多台计算机的资源被视为分布式内存群集，并使用FCFS（先到先得伺服）调度策略或其变体作为空间共享系统进行管理。
 *
 * 从GridSim 2.2开始，可以将其他调度算法添加到外部（无需编译或替换现有GridSim JAR文件）到Grid资源中。
 * 有关更多信息，请参见教程页面或AllocPolicy类。
 *
 * 从GridSim 3.0开始，可以在外部创建不同类型的资源，而无需修改此类。您需要执行以下操作：
 *
 * 从这个阶层延伸
 * 仅使用GridResource（String，double，ResourceCharacteristics，ResourceCalendar，AllocPolicy）中的构造函数
 * 重写registerOtherEntity（）方法向GridInformationService注册其他实体或标签。
 * 但是，您还需要创建一个从GridInformationService扩展的新子类。
 * 覆盖processOtherEvent（Sim_event）方法以处理除标准标记外的其他传入标记。
 *
 * 注意：
 * 如果执行步骤3，则不需要重写body（）方法。
 * 可以使用Sim_entity类中的super.get_name（）方法找到资源名称。
 * 可以使用Sim_entity类中的super.get_id（）方法找到资源ID。
 * 从GridSim 3.1开始，此模拟已包含一个网络框架。要使用此资源，您仅需要使用以下构造函数创建资源实体：
 *
 * GridResource（字符串，链接，ResourceCharacteristics，ResourceCalendar，AllocPolicy）
 * GridResource（字符串，链接，ResourceCharacteristics，ResourceCalendar）
 * GridResource（String，Link，long，ResourceCharacteristics，double，double，double，LinkedList，LinkedList）
 * 然后，您需要将此实体附加到整个网络拓扑中，即将该实体连接到路由器等。有关更多详细信息，请参见示例。
 *
