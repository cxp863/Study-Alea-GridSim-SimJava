 ...
 *  </ul>
 *
 * ParallelResource扩展了GridResource类，以获得通信和并发实体功能。
 * 此类的实例模拟具有在ResourceCharacteristics类的对象中定义的属性的资源，以保持与GridSim的兼容性。
 * 要创建并行资源，您需要具有PE（处理元素）的机器列表，这些机器具有适当的MIPS（每秒百万条指令）或SPEC（标准性能评估公司）等级；
 *
 * 注意：
 * 包含此资源的计算机列表必须是同类的。
 * 不考虑资源本地负载，资源日历将不可用于分配策略。
 * 如果要模拟本地负载，则必须将本地负载建模为用户提交的作业。 它更精确，更快。 这样做，请检查WorkloadLublin99。
 *
