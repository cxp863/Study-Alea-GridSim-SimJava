 ...
 *
 * GridSim ResourceDynamics类表示资源的静态属性，例如资源体系结构，操作系统（OS），
 * 管理策略（具有预先保留的时间共享，空间共享，并行空间共享和并行空间共享），
 * 成本和时间 资源沿资源配置位于的区域。
 * 注意：创建此类的目的是为了与GridSim模型保持兼容，在GridSim模型中，
 * ResourceCharacteristics实现的方法可返回可用PE的数量以及其他信息。
 *
