 ...
 * entity.
 *
 * GridSimShutdown等待所有GridSim用户实体的终止以确定模拟的结束。
 * 此类将在初始化模拟时由GridSim创建，即通过GridSim.init（）方法完成。 因此，无需担心创建此类的对象。
 *
 * 此类通知用户定义的报告编写者实体与GridStatistics实体进行交互以生成报告。
 * 最后，它向GridInformationService（GIS）实体发出模拟结束的信号。
 *
