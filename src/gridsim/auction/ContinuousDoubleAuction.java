 ...
 *
 * 该类代表Continuos Double Auction。 在连续两次拍卖中，拍卖师会匹配要价和出价。
 * 拍卖师维护以递增顺序排列的要求清单和以递减顺序排列的投标清单。
 * 拍卖人收到要价时，将进行以下操作：1.将其与清单的第一出价进行比较。
 * 如果要价中的价格低于或等于买价，它会通知卖方和投标人可以按价格进行交易（价格要价+价格要价）/ 2）2。
 * 否则，拍卖师将要价添加到列表中。 如果拍卖师收到投标书，它将执行以下操作：
 * 1.将其与清单中的第一个要价进行比较。 如果要价中的价格低于或等于买价，它会通知卖方和投标人可以按价格进行交易（要价+买价/ 2）。
 * 2.否则，拍卖师将出价添加到列表中。
 *
