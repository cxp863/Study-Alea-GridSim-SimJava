 ...
 *
 * GridSim Random提供用于将随机性纳入任何模拟数据的静态方法。
 * 注意从版本5.0开始，初始化模拟后GridSim将不会创建此类。 因此，您应该根据需要实例化此类。
 *
 * 任何预测或估计的数据，例如 实验所使用的Gridlet的数量，
 * Gridlet的执行时间和输出大小等需要在预测或估计过程中呈现，并且自然本身存在随机性。
 *
 * 示例：要生成介于18.00和22.00之间的随机数，需要调用GridSimRandom.real（20.0，0.1，0.1，randDouble），
 * 其中randDouble来自java.util.Random。
 *
