 ...
 *
 * GridSim ResGridlet表示提交给GridResource进行处理的Gridlet。
 * 此类跟踪特定Gridlet的GridResource中所有活动的时间。
 * 在Gridlet退出GridResource之前，建议调用此方法finalizeGridlet（）。
 * 它包含一个Gridlet对象及其到达时间，机器ID和分配给它的PE（处理元素）。
 * 它充当占位符，用于维护在不同时间分配的资源份额量，以使用内部事件模拟任何调度。
 *
