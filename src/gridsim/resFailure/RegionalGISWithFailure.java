 ...
 *
 * RegionalGISWithFailure基于RegionalGIS，但具有附加的故障功能。
 * RegionalGISWithFailure是一个简单的区域GridInformationService（GIS）实体，
 * 它执行基本功能，例如存储本地资源列表以及向其他区域GIS实体询问资源。
 * 如果要实现其他复杂功能，则需要扩展此类并重写processOtherEvent（Sim_event）和/或registerOtherEntity（）方法。
 *
 * @author  Agustin Caminero and Anthony Sulistio
