 ...
 * </ul>
 *
 * 这是处理高级预留功能的资源调度程序。该调度程序还能够处理提交的作业，而无需首先保留。
 * 就像该类别的名称所建议的那样，此调度程序只能处理一些基本的AR功能，例如：
 *
 * 处理新的高级预订功能
 * 处理新的即时预订功能
 * 处理取消预订功能
 ...
 * 此调度程序有一些限制：
 *
 * 在整个保留期内，每个PE只能处理1个Gridlet。这意味着，如果预订预订3个PE，则它应仅提交3个Gridlet。
 * 如果发送预订，例如5个Gridlet，其余2个Gridlet在提交保留阶段将被忽略。
 * 如果空的PE可用，则无法将Gridlet拆分为多个PE。
 * 在一段时间内无法列出繁忙时间和空闲时间。
 * 无法修改现有预订。
 *
