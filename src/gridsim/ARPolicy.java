 ...
 * receiver's side.
 *
 * ARPolicy是一个抽象类，用于处理与Advanced Reservation功能相关的内部GridResource分配策略。
 * 通过扩展此类并实现所需的抽象方法，可以将新的调度算法添加到GridResource实体中。 还需要实现AllocPolicy抽象方法。
 * 选择的所有实现细节和数据结构均取决于子类。 所有受保护的方法和属性都可用来简化代码编写。
 * 子类应使用ReplyXXXReservation（）方法，其中XXX =取消/提交/查询/时间/创建。
 * 这是因为这些方法将正确的格式或消息发送到AdvanceReservation类。 发送不正确的消息将导致接收方发生异常。
 *
