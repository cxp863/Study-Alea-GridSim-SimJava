 ...
 *
 * 这是一个实用程序类，它可以解析文件并自动构建网络拓扑。
 * 定义网络的文件具有以下形式：
 *
 * ＃指定路由器数量
 * 路由器数量
 *
 * ＃指定每个路由器的名称和（可选）日志记录功能
 * router_name1 [true / false]
 * router_name2 [true / false]
 * router_name3 [true / false]
 * ... //其他路由器名称
 *
 * ＃链接两个路由器。 注意：路由器名称区分大小写！
 * router_name1 router_name2波特率（GB / s）prop_delay（ms）mtu（字节）
 * router_name1 router_name3波特率（GB / s）prop_delay（ms）mtu（字节）
 * ... //链接其他路由器
 *
 * 注意：[]是用于记录路由器内部活动的可选参数。 如果未指定，则默认值为false。
 *
