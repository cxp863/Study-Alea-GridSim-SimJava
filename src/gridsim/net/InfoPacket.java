 ...
 *
 * InfoPacket类可用于从网络层收集信息。
 * InfoPacket类似于NetPacket遍历网络拓扑，
 * 但是它收集诸如带宽，往返时间等信息。它等效于物理网络中的ICMP。
 * 您可以将所有参数设置为可应用于NetPacket的InfoPacket。
 * 因此，如果要查找特定类型的NetPacket遇到的信息的种类，
 * 请将InfoPacket的大小和网络类别设置为与NetPacket相同，
 * 然后将其从相同的源发送到相同的目的地。
 *
