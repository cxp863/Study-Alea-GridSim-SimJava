 ...
 *
 * 仅将此路由器与活动数据包调度程序（例如RateControlledScheduler实体）结合使用。
 * 此类使用RIP形式实现路由的路由器。 此处使用的路由协议类似于路由信息协议（RIP）。
 * 可以在提交Gridlets等之前运行路由协议。
 *
 * 如果到目的地的路由多于两条，则使用跳数较低的路由。
 * 由于在此仿真中，路由器中继完美的信息并且链路不会中断，因此RIP应该是可靠的协议。
 *
 * @invariant $none
