 ...
 * </ul>
 *
 * 此类实现路由器，该路由器将数据从一个实体转发到另一个实体。
 * 该抽象类仅包含用于连接其他路由器和主机的抽象方法。另外，该类中的常用功能可供其子类使用。
 * 但是，此类未提供有关如何设置转发表的方法，因此如果用户要创建具有某些特定路由算法的路由器，则该设计留给用户。
 *
 * 从此类扩展时，要考虑的几个重要注意事项：
 *
 * 不要覆盖body()方法，因为它包含将这个实体注册到GridInformationService实体并在退出模拟之前完成日志记录信息的代码/功能。
 * 必须重写advertiseHosts()方法。需要将与该实体连接的所有主机或实体发布到相邻路由器。
 * registerOtherEntity()方法：用于将其他事件类型/标记注册到GridInformationService。这是可选的。
 * 必须重写processEvent(Sim_event)方法。处理传入事件需要它。
 * 不要忘记为传入和传出数据包实现日志记录或记录功能。
 * SimReport对象是在此类中创建的，因此，您只需要使用write(String)方法。
 *
 * @invariant $none
