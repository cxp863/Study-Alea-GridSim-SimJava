 ...
 *
 * SCFQScheduler实现了自时钟公平排队调度程序。
 * SCFQ是加权公平排队（WFQ）的一种变体，它比WFQ易于实现，因为它不需要在每次迭代时都计算轮数。
 * 有关更多详细信息，请参阅S. R. Golestani的INFOCOM '94论文“宽带应用的自计时公平排队方案”。
 * SCFQ调度程序可以通过更改与某类流量关联的权重，为流量提供差异化​​服务。
 * 一类交通的权重越高，它得到的待遇就越好。在此类中，可以通过使用线性权重数组调用setWeights()来设置权重。
 * 为类别0（默认）的流量分配了数组的第一个元素作为其权重。
 *
 * 例如：
 * 字符串userName = {“ User_0”，“ User_1”，User_2“}; //用户名列表
 * int [] trafficClass = {0，1，2}; //每个用户的类别列表
 * int []权重= {1，2，3}; //每个类别的权重列表
 *
 * 从上面的示例中可以看出，User_0的流量类别为0（低优先级），而User_2的流量类别为2（高优先级）（从它们各自的权重来看）。
 *
 * @invariant $none
