 ...
 *
 * RateControlledScheduler是一个速率抖动控制调节器的实现。
 * 有关更多详细信息，请参阅H. Zhang和D.Ferrari的INFOCOM '93论文“速率控制的静态优先级队列”。
 * RateControlledScheduler可用于控制在路由器上分配给每个用户类别的带宽。
 * 这是一种不工作的保留算法，这意味着即使队列中有数据包，路由器也可以保持空闲状态。
 *
 * 在RateControlledScheduler上，为每个类别的用户分配了一定百分比的带宽，并且调度程序确保每个类别始终保持在其带宽限制内。
 *
