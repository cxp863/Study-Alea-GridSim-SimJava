 ...
 *
 * 此类基于SpaceShared，但具有附加的故障功能。
 * PerfectCheckPointing类是GridResource的分配策略，
 * 其行为与“先来先服务（FCFS）”完全一样。
 * 这是一个基本且简单的调度程序，它将每个Gridlet运行到一个处理元素（PE）。
 * 如果Gridlet需要多个PE，则此调度程序仅将此Gridlet分配给一个PE。
 * 在发生故障事件的情况下，将在故障计算机上暂停网格。
 * 在发生恢复事件的情况下，将恢复已恢复机器上的网格。
 *
