 ...
 *
 * SimJava仿真内核。
 * 此类负责管理模拟的所有方面。实例化的实体被添加到Sim_system中，该​​实体负责同步其行为。时间根据事件安排时间而变化。
 * 这些事件保存在两个队列中，即Evqueue实例，未来队列和延迟队列。实体可用的所有运行时功能均由Sim_system管理。
 *
 * 在以前的SimJava版本中，用户只能使用Sim_system初始化仿真，添加实体，链接其端口并最终运行仿真。
 * 在2.0版中，Sim_system为建模者提供了更多功能。
 *
 * Sim_system的三个新方面是瞬态条件，终止条件的定义以及要应用的输出分析方法的选择。所有这些都由Sim_system内部管理，
 * 因此不需要用户在仿真实体中包括相关代码。为了设置仿真的瞬态条件，用户使用set_transient_condition方法。
 * 通过使用set_termination_condition方法设置模拟的终止条件。最后，使用set_output_analysis方法之一选择一种输出分析方法。
 *
 * 可用的瞬态条件是：
 *
 * 许多事件的完成。在这种情况下，在给定实体完成一定数量的事件服务后，模拟被视为已进入稳态。
 * 使用实体的sim_completed方法将事件完成通知给Sim_system。
 * 特定时间段的模拟时间。经过这段时间后，模拟被视为已进入稳态。
 * 最小最大方法。此方法允许Sim_system尝试根据给定度量的最小-最大方法来识别瞬态周期。一旦找到了一个测量值观察值，
 * 该状态值既不是剩余观察值的最小值也不是最大值，则此方法认为已进入稳态。
 * 没有。如果未指定任何条件，则为默认设置。
 * 可用的终止条件为：
 *
 * 许多事件的完成。一旦给定数量的事件完成了给定实体的服务，模拟就会终止。此条件类似于相关的瞬态条件。
 * 特定时间段的模拟时间。经过这段时间后，仿真将终止。
 * 度量的置信区间准确性。该方法允许Sim_system运行仿真，直到给定度量的置信区间达到某个准确度级别为止。要使用此方法，
 * 用户需要提供感兴趣的度量，一种将用作方差减少技术的输出分析方法，置信度和所需的精度。准确性是置信区间的半角宽与量度的总平均值之比。
 * 没有。如前所述，如果未指定条件，则为默认设置。非终止模拟仅出于动画目的才有意义，在这种情况下，模拟被用作系统的详尽演示。
 * 实体可以使用运行方法来检查模拟是否已完成。
 *
 * 最后，可用的输出分析方法是：
 *
 * 独立复制。在这种情况下，模拟运行会重复多次以产生总体测量结果。
 * 批处理方式。使用这种方法，需要执行一次长期运行，然后将获得的观测结果分批处理，然后使用其平均值进行总测量。
 * 所使用的批次数量是使批次平均值之间的序列相关性最小的数量。对于大多数措施。
 * 没有。同样，这是默认方法。在这种情况下，不执行输出分析，仅产生可用的样本测量值。
 *
 * 从SimJava 2.0版开始，Sim_system还允许建模者更好地控制随机数生成器的种子。
 * 可以以允许Sim_system自动为它们播种的方式创建模拟的生成器，从而确保生成器的空间均匀。
 * 可以使用set_seed_sequence方法设置要使用的原始种子以及所需的种子间距。如果不使用此方法，
 * 则使用默认种子4851来生成间隔为100000个样本的种子。如果将独立复制用作输出分析方法，
 * 并且希望在每个后续复制中重新生成生成器，则需要将生成器添加到其实体中。这是通过实体的add_generator方法完成的。
 * 否则，将先前复制的最后一个种子用作下一个复制的初始种子。
 *
 * Sim_system还提供对模拟输出的控制。可以更详细地管理跟踪文件（sim_trace）。
 * set_trace_detail方法可用于指定是否添加默认跟踪，实体生成的跟踪或特定事件类型的跟踪。
 * 在后一种情况下，可以使用track_event方法将感兴趣的事件标签提供给Sim_system。
 * 可以使用set_trace_level方法管理跟踪文件中包含的实体跟踪级别。
 * 最后，可以使用set_auto_trace方法一起关闭跟踪。默认为不生成跟踪文件。
 *
 * 模拟输出的另一个主要方面是模拟报告。该报告由Sim_system自动生成，并且包含有关模拟的信息，具体取决于指定的条件和参数。
 * 可以使用set_report_detail方法修改报告文件的详细程度（sim_report）。包括的默认信息是获得的总测量值以及有关模拟运行的一般信息。
 * 在未使用输出分析方法的情况下，将以稳态样品测量值代替总测量值。
 * 如果使用了输出分析方法，则可以使用set_report_detail包括每个复制的样本测量值（对于独立复制），
 * 或每个批次的样本测量值（对于批次平均值）。最后，模拟的随机数生成器中使用的原始种子也可以包含在报告中。
 * 从2.0版开始，仿真报告也可用于动画仿真。
 *
 * generate_graphs方法也可以用于生成仿真图。将为所有详细的度量生成图形，并在模拟过程中显示每个度量的样本平均值的进度。
 * 还可以为每个图形显示几个其他信息，例如平均置信区间，瞬态周期，批次等。模拟将产生一个包含模拟数据的“ .sjg”文件。
 * 然后可以使用SimJava Graph Viewer（用于SimJava仿真的图形查看实用程序）打开该文件。
 *
 * 有关模拟的定义和Sim_system的使用的更多信息，请参见《 SimJava教程》。
 *
 * @see         Sim_entity
...
public class Sim_system {
  ...
  // ------coding by decompiler----------------------------------------------------------------
  public static List<Sim_entity> getEntityList()
  {
    LinkedList<Sim_entity> list = new LinkedList();
  ...
  public static void setInComplete(boolean paramBoolean) {
