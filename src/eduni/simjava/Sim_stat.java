 ...
 *
 * 如果建模者对从实体收集统计度量感兴趣，则需要创建Sim_stat对象，定义关注度量并将其设置为实体的统计收集器。
 * Sim_system也可以访问仿真中定义的Sim_stat对象，以便应用输出分析以及某些瞬态和终止条件。
 *
 * 向实体添加统计支持的第一步是在实体的构造函数中创建Sim_stat对象。此后，将调用add_measure方法来定义实体的关注度量。
 * 对于感兴趣的每个度量重复此操作。量度的定义方式取决于其类型。度量可以是默认度量或自定义度量。
 *
 * 默认度量是大多数模拟中都可以考虑的度量。由于它们的含义很简单，因此可以根据模拟中的内部事件自动收集这些观测值。
 * 建模者所需要做的就是定义感兴趣的默认度量，然后让Sim_system根据在仿真报告中的观察结果来包括摘要统计信息。
 * 建模者还可以通过几种度量计算方法在运行时访问度量。可以从默认度量获取的度量取决于度量的类型。
 *
 * 另一方面，自定义度量是具有模拟特定含义的度量。它们通常不适用，因此建模者需要定义其类型以及将在其上进行更新的实例。
 * 除了自定义度量的定义及其更新之外，它们与默认度量没有任何区别。
 *
 * 所有度量都可以归为以下类别之一：
 *
 * 基于费率的度量。这些度量基于一段时间内的事件发生。基于比率的度量的示例可以是实体的到达率，丢失率和吞吐量。
 * 基于连续状态的度量。基于状态的度量基于给定时间段内具有特定级别的实体。
 * 基于连续状态的度量是指实体不断具有一定级别或处于某个状态的度量。此类度量的示例是实体的队列长度及其利用率。
 * 基于非连续状态的度量。这些度量不同于连续度量，因为不需要实体始终处于某种状态。
 * 用户可以将基于非连续状态的度量定义为自定义度量。没有提供此类默认措施。
 * 基于时间间隔的度量。这些度量基于实体或实体所经历的事件所经历的时间间隔。
 * 基于间隔的度量的一个示例是某个实体上事件的平均等待时间。
 * 提供的默认度量如下：
 *
 * 到达率（基于费率）
 * 吞吐量（基于费率）
 ...
 *
 * 基于费率：平均值，事件计数
 * 基于状态（连续和非连续）：平均值，方差，标准差，最小值，最大值，超出比例。
 * 超出比例用于计算实体状态高于每个给定状态的时间比例。请注意，对于利用率而言，
 * 只有平均值可用，因为SimJava中的实体被认为具有单一服务器行为。
 * 基于时间间隔：平均值，方差，标准差，最小值，最大值，超出比例。在这种情况下，
 * 超出比例用于计算在每个给定级别上经历例如等待时间的事件的比例。
 * 这些测量是通过Sim_stat的相关方法产生的。用户可以在运行时调用这些命令以检查度量的进度，
 * 或者在Simmsystem的仿真结束时调用它们，以应用输出分析并生成仿真的报告。
 *
 * 为了计算超出比例，在定义了相关度量后，用户可以在Sim_stat对象上调用calc_proportions。
 * 此方法提供了要计算比例的兴趣级别。
 *
 * 用户还可以指定某些事件标签，应考虑为其定义的措施。这可以通过measure_for方法实现。
 * 选择感兴趣的标签的能力很重要，因为带有特定标签的事件通常用作实体之间的控制事件，
 * 并不代表实际的系统作业。此外，可能是标签在不同类型的事件之间进行区分的情况，
 * 在这种情况下，用户可能只对那些类型之一的度量感兴趣。指定的标签用于决定是否应收集观测值以进行默认测量。
 * 如果是自定义措施，则不考虑指定的标签。之所以如此，是因为自定义度量由建模者显式更新。
 *
 * 自定义度量也可以用于为几种不同的事件类型实施默认度量。在这种情况下，
 * 用户为每种感兴趣的事件类型定义单独的度量，并在模拟运行期间着手对其进行更新。
 *
 * 除了将度量定义为默认度量或自定义度量外，用户还可以选择有关其计算效率的其他选项。
 * 计算测量的默认方式是通过观测收集。每当使用内部（用于默认度量）或称为更新方法（用于自定义度量）的用户时，
 * 都会根据给定的参数收集观察值。为每个度量收集观测值的方法可提供最大可能的信息级别。
 * 此类详细的度量可在所有瞬态和终止条件下与所有输出分析方法一起使用，还可用于生成度量随时间变化的图表。
 * 但是，在这种情况下，存在相当大的内存需求，在某些情况下这可能是禁止的。
 *
 * 因此，用户可以选择将度量定义为“有效”。在这种情况下，不会收集观测值，但是每次更新都会动态地重新计算测量值。
 * 这种方法最大程度地减少了内存需求，将用于具有大量实体和度量的大型模拟情况下，这些实体和度量将具有较大的运行长度。
 * 在这种情况下，权衡是功能。当定义了有效措施时，批次方法就不能用作输出分析方法。
 * 此外，瞬态周期估计的最小-最大方法不能与有效措施一起使用。方差和标准偏差的度量不适用于这些度量，
 * 而其他度量只能针对整个运行长度或整个稳态进行计算，而不能针对任意时间段进行计算。最后，可能无法生成有效措施的图形。
 *
 * 用户应自行决定使用哪种方法。但是，可能存在上述限制的详细有效措施的混合。
 * 在建立Sim_stat对象时，通过调用set_efficient方法将一种措施指定为有效。
 *
 * 最后要考虑的是Sim_stat对象的设置步骤的执行顺序：
 *
 * Sim_stat对象被实例化。
 * 定义了默认和自定义度量。对于将计算超出比例的每个度量，将在度量定义之后调用calc_proportions方法。
 * 使用set_efficient方法指定有效措施。这里的要点是，必须在定义措施之后以及指定超出比例之后将其设置为有效措施。
 * 定义了感兴趣的标签。这可以在任何阶段完成。
 * 最后，使用set_stat方法将Sim_stat对象设置为实体的统计信息收集器。
 * 有关SimJava统计支持的更多信息，请参见SimJava教程。
 *
 *
 * @see Sim_system
 * @see Sim_entity
